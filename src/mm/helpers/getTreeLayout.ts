...
const getNodeLayout = (canvas2D: CanvasRenderingContext2D, node: TreeNodeWithTreeSize, canvasSize: { width: number, height: number }): TreeNodeWithLayout => {
  ...
  // coordinate at middle-center
  //const x = (canvasSize.width * CANVAS_SCALE_FACTOR) / 2 - fullTreeWidth / 2;

  // coordinate at middle-left
  const x = ((canvasSize.width * CANVAS_SCALE_FACTOR) / 2) * 0.1; // 10% from the left side
  const y = (canvasSize.height * CANVAS_SCALE_FACTOR) / 2;
...
export const getTreeLayout = (
  ...
  const withLayout = getNodeLayout(canvas2D, withTreeSizes, canvasSize);
